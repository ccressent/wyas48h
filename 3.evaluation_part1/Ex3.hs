...
-- Note that the pattern in not exhaustive and that there is no error handling.
-- I think it's the theme of the next chapter.
-- Also, even after reading the relevant sections in R5RS, I'm not sure how to
-- transform a string with spaces in it into a Symbol, as Symbol names can't
-- have spaces in them. Maybe it should just be an error, or try to truncate?
